...
export function CardOne() {
    return (
        <div className='grid grid-cols-1 sm:grid-cols-2 sm:gap-12 lg:grid-cols-4 gap-4 place-content-evenly m-8'>
            {cardData.map(card => (
                <div key={card.id} className="relative h-[300px] w-[225px] rounded-md   items-center justify-center mx-auto">
                    <img
                        src={card.image}
                        alt="Card Image"
                        className="z-0 h-full w-full rounded-md object-cover"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-gray-900 to-transparent hover:from-red-900 "></div>
